...
function editToDoItem(category, arrayNumber, mode = "edit") {
  ...
  const toDoDescriptionInput = modal.querySelector(
    'input[name="descriptionEdit"]'
  );
...
function displayToDos(category, arrayNumber, mode = "add") {
  ...
  if (mode == "add") {
    ...
    toDoListItemName.innerText = `Title: ${
      category.getToDoList()[arrayNumber].toDoName
    }`;
    toDoListItemDescription.innerText = `Description: ${
      category.getToDoList()[arrayNumber].description
    }`;
    toDoListItemDueDate.innerText = `Due Date: ${
      category.getToDoList()[arrayNumber].dueDate
    }`;
    toDoListItemPriority.innerText = `Priority: ${
      category.getToDoList()[arrayNumber].priority
    }`;

    toDoListItemName.innerText != "undefined"
      ? rightSection.appendChild(toDoListItem.appendChild(toDoListItemName))
      : null;
    toDoListItemDescription.innerText != "undefined"
      ? rightSection.appendChild(
          toDoListItem.appendChild(toDoListItemDescription)
        )
      : null;
    toDoListItemDueDate.innerText != "undefined"
      ? rightSection.appendChild(toDoListItem.appendChild(toDoListItemDueDate))
      : null;
    toDoListItemPriority.innerText != "undefined"
      ? rightSection.appendChild(toDoListItem.appendChild(toDoListItemPriority))
      : null;
    toDoListItemName.innerText != "undefined"
    ...
    const toDoListItemDescription = document.querySelector(
      "#toDoListItemDescription"
    );
    const toDoListItemDueDate = document.querySelector("#toDoListItemDueDate");
    const toDoListItemPriority = document.querySelector(
...
currentCategory.displayToDoList();
let first = general.toDoListItem(
  "Call Doc",
  "call 716-111-1111",
  "Tomorrow",
  "High"
);
let second = general.toDoListItem(
  "Email Sheryl",
  "Telling her about the fundraiser",
  "Today",
  "Low"
);
